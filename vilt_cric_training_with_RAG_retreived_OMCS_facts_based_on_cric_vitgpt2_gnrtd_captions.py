...
1. Training the vilt-classification model by attaching the omcs_facts as external knowledge.
2. The retrieval process of omcs_facts is based on the Image captions of CRIC and not based on the knowledge_fact provided with CRIC.
3. VitGPT2 captioning engine was used to caption the images of CRIC.
4. In this script the image_caption is being imported from an already stored caption_text_files generated with vitgpt2 model.
"""
...
path = './vilt_omcs_trained_captions_main.txt'
...
# Importing Training Set Captions

print('\nImporting Image Captions From cricVitGpt2CaptionsTrainSet.txt File')

trainingCaptions = []

with open('cricImageCaptions/cricVitGpt2CaptionsTrainSet.txt') as textfile:
    for line in textfile:
        caption = line.strip().split('_')[1]
        
        trainingCaptions.append(caption)


# Attaching the top 1-fact before the question based on Image_Caption

print('\nAttaching 1-Knowledge Fact Based on attached Image Caption')

for i in tqdm(range(len(questionList))):

    currentQuestion = questionList[i]
    caption = trainingCaptions[i]
    
    factsToBeAttached = findTop1RelevantFacts(caption)
...
# Importing Validation Set Image Captions

print('\nImporting Image Captions of the Validation Set')

ValCaptions = []

with open('cricImageCaptions/cricVitGpt2CaptionsValSet.txt') as textfile:
    for line in textfile:
        caption = line.strip().split('_')[1]
        
        ValCaptions.append(caption)

# Attaching Top-1 fact before the questions of val set based on the Image Captions

print('\nAttaching 1 Knowledge fact before the questions based on the Image Caption')

for i in tqdm(range(len(questionList_val))):

    currentQuestion = questionList_val[i]
    caption = ValCaptions[i]
...
# Importing Test Set Image Captions

print(f'\nImporting Validation Set Image Captions')

TestCaptions = []

with open('cricImageCaptions/cricVitGpt2CaptionsTestSet.txt') as textfile:
    for line in textfile:
        caption = line.strip().split('_')[1]
        
        TestCaptions.append(caption)

# Attaching 1 knowledge fact from OMCS with the Questions based on the image captions

print(f'\nAttaching 1-knowledge fact with the Questions based on the Image Captions')

for i in tqdm(range(len(questionList_test))):

    currentQuestion = questionList_test[i]
    caption = TestCaptions[i]
...
for epoch in tqdm(range(10)):  
    ...
    save_path = os.path.join('./model_chkpts/vilt_omcs_facts_trained_based_on_vitgpt2_caption/', 'vilt_train_on_caption_mod_' + str(epoch+1) + '_KG')
